...
class PackageLoader:
    ...
    def _talent_prompt_path(self, template_id: str) -> Path:
        # Replace colons with '--' for filesystem safety (Windows NTFS forbids colons in filenames)
        safe_id = str(template_id or "").strip().replace(":", "--")
        filename = re.sub(r"[^A-Za-z0-9._-]+", "-", safe_id).strip("-") or "template"
