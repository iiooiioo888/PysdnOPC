...
def _translate_progress_prefix(text: str) -> str:
    ...
    if text.startswith("[Delegating to "):
        inner = text[len("[Delegating to "):]
        if inner.endswith("]"):
            inner = inner[:-1]
        # Extract agent name (first token before space or ])
        agent = inner.split("]")[0].split(" ")[0] if "]" in inner else inner.split(" ")[0]
        rest = inner[len(agent):]
        return f"[{t('cli.progress.delegating', agent=agent)}{rest}]"
    if text.startswith("[Autonomy]"):
        return f"[{t('cli.progress.autonomy')}]{text[10:]}"
    if text.startswith("[External status]"):
        return f"[{t('cli.progress.external_status')}]{text[17:]}"
    if text.startswith("[External approval]"):
        return f"[{t('cli.progress.external_approval')}]{text[19:]}"
    if text.startswith("[External agent denied]"):
        return f"[{t('cli.progress.external_denied')}]{text[23:]}"
    if text.startswith("[External agent failed]"):
        return f"[{t('cli.progress.external_failed')}]{text[23:]}"
    if text.startswith("[External agents exhausted]"):
        return f"[{t('cli.progress.external_exhausted')}]{text[27:]}"
    return text
...
def _progress_callback():
    ...
    async def callback(text: str, **_: Any) -> None:
        if text.startswith("[Tool:"):
            ...
            console.print(Panel(_translate_progress_prefix(text), title=t("cli.progress.suggestion"), border_style="yellow"))
        elif text.startswith("[Delegating") or text.startswith("[Autonomy]"):
            console.print(f"  [info]{escape(_translate_progress_prefix(text))}[/info]")
            ...
            console.print(f"  [warning]{escape(_translate_progress_prefix(text))}[/warning]")
...
@app.command()
def chat(
    message: Optional[str] = typer.Argument(None, help=t("cli.opt.message")),
    project: Optional[str] = typer.Option(None, "--project", "-p", help=t("cli.opt.project")),
    model: Optional[str] = typer.Option(None, "--model", "-m", help=t("cli.opt.model")),
    mode: str = typer.Option("task", "--mode", help=t("cli.opt.mode")),
    agent: Optional[str] = typer.Option(None, "--agent", help=t("cli.opt.agent")),
    company_profile: str = typer.Option("corporate", "--company-profile", help=t("cli.opt.company_profile")),
...
def exec_command(
    ...
    company_profile: str = typer.Option("corporate", "--company-profile", help=t("cli.opt.company_profile")),
    agent: Optional[str] = typer.Option(None, "--agent", help=t("cli.opt.agent")),
    org_id: Optional[str] = typer.Option(None, "--org", "--org-id", help=t("cli.opt.org_id")),
    session_id: Optional[str] = typer.Option(None, "--session-id", help=t("cli.opt.session_id")),
    resume: bool = typer.Option(False, "--resume", help=t("cli.opt.resume")),
    json_output: bool = typer.Option(False, "--json", help=t("cli.opt.json_output")),
    ...
    if session_id and resume:
        console.print(f"[error]{t('cli.error.session_or_resume')}[/error]")
        raise typer.Exit(code=2)
    message = prompt
    ...
    try:
        ...
        if stream_json:
            ...
            console.print(f"\n[warning]{t('cli.interrupted')}[/warning]")
        raise typer.Exit(code=130) from exc


@app.command()
def secretary(
    message: Optional[str] = typer.Argument(None, help=t("cli.opt.secretary_message")),
    project: Optional[str] = typer.Option(None, "--project", "-p", help=t("cli.opt.project")),
    model: Optional[str] = typer.Option(None, "--model", "-m", help=t("cli.opt.model")),
    verbose: bool = typer.Option(False, "--verbose", "-v", help=t("cli.opt.verbose")),
    no_markdown: bool = typer.Option(False, "--no-markdown", help=t("cli.opt.no_markdown")),
...
@app.command()
def init(
    project: Optional[str] = typer.Argument(None, help=t("cli.opt.project")),
    yes: bool = typer.Option(False, "--yes", "-y", help=t("cli.opt.yes_init")),
    external_agent_preflight: bool = typer.Option(
        True,
        "--external-agent-preflight/--no-external-agent-preflight",
        help=t("cli.opt.external_agent_preflight"),
    ),
    ...
    if already_initialized:
        if not yes and not typer.confirm(
            t("cli.init.already_initialized", path=str(opc_home)),
            default=False,
        ):
            console.print(f"[warning]{t('cli.init.cancelled')}[/warning]")
            raise typer.Exit(1)
        console.print(f"[info]{t('cli.init.config_preserved', path=str(opc_home / 'config'))}[/info]")
    ...
    if project:
        if not re.match(r"^[a-zA-Z0-9][a-zA-Z0-9_-]*$", project or ""):
            console.print(f"[error]{t('cli.init.invalid_project_id')}[/error]")
            raise typer.Exit(1)
        ...
        if proj_dir.exists() or project_memory.exists() or workplace.exists():
            console.print(f"[error]{t('cli.init.project_exists', project=project)}[/error]")
            raise typer.Exit(1)
        ...
        console.print(f"[success]{t('cli.init.project_created', project=project, path=str(proj_dir))}[/success]")
    ...
    console.print(f"[success]{t('cli.init.success', path=str(opc_home))}[/success]")
    console.print(f"  Config: {opc_home / 'config'}")
    console.print(f"  Memory: {opc_home / 'memory'}")
    console.print(f"  Skills: {opc_home / 'skills'}")
    console.print(f"\n{t('cli.init.edit_api_key', path=str(opc_home / 'config' / 'llm_config.yaml'))}")


@app.command()
def status(
    project: str = typer.Option("default", "--project", "-p", help=t("cli.opt.status_project")),
    external_agent_preflight: bool = typer.Option(
        True,
        "--external-agent-preflight/--no-external-agent-preflight",
        help=t("cli.opt.external_agent_preflight"),
    ...
    # Roles
    console.print(f"\n[bold]{t('cli.status.organization')}[/bold]")
    ...
    # External agents
    console.print(f"\n[bold]{t('cli.status.external_agents')}[/bold]")
    for name, agent_config in config.agents.agents.items():
        status_str = f"[success]{t('cli.status.enabled')}[/success]" if agent_config.enabled else f"[warning]{t('cli.status.disabled')}[/warning]"
        console.print(f"  - {name}: {status_str}")

    if not config.agents.agents:
        console.print(f"  {t('cli.status.none_configured')}")

    console.print(f"\n[bold]{t('cli.status.preflight')}[/bold]")
    if external_agent_preflight:
        from opc.layer3_agent.preflight import run_external_agent_preflight
    ...
    if projects_dir.exists():
        ...
        console.print(f"\n[bold]{t('cli.status.projects')}[/bold] {', '.join(projects) if projects else '(none)'}")
...
session_app = typer.Typer(help=t("cli.sub.session"))
...
mode_app = typer.Typer(help=t("cli.sub.mode"))
...
kanban_app = typer.Typer(help=t("cli.sub.kanban"))
kanban_task_app = typer.Typer(help=t("cli.sub.kanban_task"))
...
agent_app = typer.Typer(help=t("cli.sub.agent"))
...
org_app = typer.Typer(help=t("cli.sub.org"))
...
org_saved_app = typer.Typer(help=t("cli.sub.org_saved"))
...
org_role_app = typer.Typer(help=t("cli.sub.org_role"))
...
org_policy_app = typer.Typer(help=t("cli.sub.org_policy"))
...
org_strategy_app = typer.Typer(help=t("cli.sub.org_strategy"))
...
runtime_app = typer.Typer(help=t("cli.sub.runtime"))
...
comms_app = typer.Typer(help=t("cli.sub.comms"))
...
work_item_app = typer.Typer(help=t("cli.sub.work_item"))
...
channels_app = typer.Typer(help=t("cli.sub.channels"))
app.add_typer(channels_app, name="channels")

talent_app = typer.Typer(help=t("cli.sub.talent"))
...
talent_employee_app = typer.Typer(help=t("cli.sub.talent_employee"))
...
# Market subcommands
# ---------------------------------------------------------------------------
market_app = typer.Typer(help=t("cli.sub.market"))
...
async def _single_message(
    ...
    *,
    ...
    try:
        ...
        console.print(f"\n[info]{t('cli.processing')}[/info] {message}\n")
...
async def _single_secretary_message(config, message: str, project: str | None, no_markdown: bool) -> None:
    ...
    try:
        await engine.initialize()
        console.print(f"\n[info]{t('cli.secretary_processing')}[/info] {message}\n")
        runtime_display.begin_turn()
...
async def _process_interactive_chat_message(
    ...
    *,
    ...
    try:
        ...
        console.print(f"\n[warning]{t('cli.interrupted')} Type /quit to exit.[/warning]")
    except Exception as e:
