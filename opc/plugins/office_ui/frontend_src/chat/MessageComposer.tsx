...
export function MessageComposer({
  ...
  const prepareAttachment = useCallback(async (attachmentId: string, file: File) => {
    try {
      ...
      updateAttachment(attachmentId, (attachment) => ({
        ...attachment,
        error: t('composer.failedToPrepare'),
        progress_percent: 0,
  ...
  const addFiles = useCallback((files: FileList | File[]) => {
    ...
    const newPending: PendingAttachment[] = arr.map(file => {
      ...
      if (file.size > MAX_FILE_SIZE) error = t('composer.tooLarge', { size: formatSize(file.size) })
      else if (runningTotal + file.size > MAX_TOTAL_SIZE) error = t('composer.totalSizeExceeds')
  ...
  const handleSend = useCallback(() => {
    ...
    onSend(content || t('composer.sentWithAttachments'), attachments.length ? attachments : undefined)
    setText('')
    ...
    const all: ModeAlternative[] = [
      {
        key: 'task',
        mode: 'task',
        label: t('composer.task'),
        description: t('composer.taskDescription'),
        icon: <IconUserRound />,
      },
      {
        ...
        mode: 'company',
        profile: 'corporate',
        label: `${t('composer.company')} / ${t('composer.corporate')}`,
        description: t('composer.companyDescription'),
        icon: <IconBuilding />,
      },
    ]
    if (continueOrgName) {
      all.push({
        ...
        label: `${t('composer.company')} / ${continueOrgLabel}`,
        description: t('composer.savedOrgDescription'),
        icon: <IconSparkles />,
      })
    }
  ...
  const statusText = (() => {
    ...
    if (isStopping) return t('composer.stopping')
    if (activeAgentCount && activeAgentCount > 1) return t('composer.agentsWorking', { n: activeAgentCount })
    if (visibleTool) return t('composer.runningTool', { tool: visibleTool })
    if (agentStatus === 'reflecting') return t('composer.thinking')
                  ...
                  >
                    ...
                    <div className="composer-mode-chip-popover" role="dialog" aria-label="Mode info">
                      <div className="composer-mode-chip-popover-title">
                        {t('composer.modeLocked')}
                      </div>
                      <div className="composer-mode-chip-popover-body">
                        {t('composer.modeLockedBody')}{' '}
...
>>>>>>> edbdb3b79d29ed68033dccda9fe79b1f7b0eb316
                  ...
                  <>
                    ...
                    {showTaskAgentPicker && !lockedMode ? (
                      <label
                        className="composer-mode-inline"
                        data-kind="agent"
                        title={t('composer.taskAgentTitle')}
                      >
                        <span className="composer-mode-inline-label">{t('composer.agent')}</span>
                        <span className="composer-mode-select-wrap">
                          <select
                            className="composer-mode-select"
            ...
            <span className="composer-hint">
              {preparingAttachmentCount > 0
                ? t('composer.preparingAttachments', { n: preparingAttachmentCount })
                : readyAttachmentCount > 0
                  ? t('composer.attachmentsReady', { n: readyAttachmentCount })
