      ...
      onAck: (payload) => {
        try {
          ...
          if (payload.ok && payload.action === 'employee_imported') {
            showToast(t('app.employeeDeployed'))
          }
          // Talent import success
          if (payload.ok && payload.action === 'talent_imported') {
            showToast(t('app.importedTemplates', { n: payload.count ?? 0 }))
          }
          // Talent hire success
          if (payload.ok && payload.action === 'talent_hired') {
            setHiringTemplateId(null)
            showToast(t('app.hiredToast', { name: payload.name ?? t('composer.agent') }))
          }
          ...
          if (payload.ok && (payload.agent_id || payload.deleted)) {
            if (payload.deleted) {
              setDeletingAgentId(null)
              setConfirmDeleteId(null)
              showToast(t('app.agentRemoved'))
            }
            if (payload.agent_id && !payload.deleted) {
              showToast(t('app.agentCreated'))
            }
          ...
          <div className="sidebar-body">
            {/* Team Panel */}
              <div className="team-panel">
                      ...
                      <div key={office.id} className="office-card" onClick={() => bridgeRef.current.panToOffice(office.id)}>
                        ...
                        <div className="office-agents">
                          ...
                          {isOrgMode && assignedCards.length < deskCount && otherAgents.length > 0 && (
                            ...
                            >
                              <option value="">{t('app.moveHere')}</option>
                              {otherAgents.map(a => (
                ...
                <div className="section-label">{t('app.activeAgents')} <span className="count-badge">{swarmAgents.length}</span></div>
                <div className="agent-list">
                        ...
                        deletingAgentId === agent.agent_id
                          ...
                          : confirmDeleteId === agent.agent_id
                            ? <span className="del-confirm">
                                <span className="del-confirm-label">{t('app.deleteConfirm')}</span>
                  ...
                  {swarmAgents.length === 0 && (
                    <div className="empty-state">{t('app.noAgents')}</div>
                  )}
                </div>
                ...
                {selectedCard && (
                  <div className="agent-detail">
                    ...
                    <div className="agent-detail-row">
                      <span className="detail-label">{t('app.office')}</span>
                      <select
                    ...
                    <div className="agent-detail-row">
                      <span className="detail-label">{t('app.seat')}</span>
                      <select
                        className="detail-select"
                ...
                {cards.length > swarmAgents.length && (
                  <>
                    <div className="section-label">
                      {t('app.characters')}
                      <button className="inline-btn" onClick={() => setShowSubagents((v) => !v)}>{showSubagents ? t('app.hideSub') : t('app.showSub')}</button>
                    </div>
                    <div className="agent-list">
      ...
      {showDevTools && (
        <div className="dev-overlay">
          <div className="dev-header">
            <span className="dev-title">{t('app.devTools')}</span>
            <button className="icon-btn" onClick={() => setShowDevTools(false)}>✕</button>
          </div>
          <div className="dev-group">
            <div className="dev-label">{t('app.connection')}</div>
          ...
          {Object.keys(snapshot?.channels ?? {}).length > 0 && (
            <div className="dev-group">
              <div className="dev-label">{t('app.channels')}</div>
