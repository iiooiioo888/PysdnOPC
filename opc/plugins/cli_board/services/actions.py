...
class BoardActions:
    ...
    async def cancel_task(self, task_id: str) -> None:
        ...
        # Directly mark related tasks as cancelled in the store
        related = await self._collect_related_tasks(task_id)
        for related_task in related:
            if related_task.status not in (TaskStatus.CANCELLED, TaskStatus.DONE):
                related_task.status = TaskStatus.CANCELLED
                await engine.store.save_task(related_task)

        # Best-effort runtime stop via office service (may fail in stub/test contexts)
        try:
