...
def test_load_via_import_format_accepted_by_loader(tmp_path):
    ...
    cfg = apply_org_architecture_snapshot(OPCConfig(), parse_org_architecture_snapshot(path.read_text(encoding="utf-8")))
...
def test_startup_restores_active_saved_org_when_org_config_empty(monkeypatch, tmp_path):
    ...
    async def run() -> None:
        ...
        on_disk = yaml.safe_load((config_dir / "company_orgs" / "org_lab_config.yaml").read_text(encoding="utf-8"))
