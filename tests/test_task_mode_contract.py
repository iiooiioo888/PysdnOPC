...
class TaskModeNativeAgentTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_task_mode_prompt_uses_unified_capability_contract(self) -> None:
        ...
        self.assertIn("你是 Task Generalist，OpenOPC 任務執行代理。", prompt)
        self.assertNotIn("一人公司", prompt)
        self.assertIn("## 核心運作原則", prompt)
        self.assertNotIn("## Core Beliefs", prompt)
        self.assertIn("## 原生工作契約", prompt)
        self.assertIn("而非由元數據選擇的 prompt 設定檔", prompt)
        self.assertIn("## 原生自我驗證契約", prompt)
        self.assertNotIn("## 任務模式編排", prompt)
        self.assertIn("## 任務模式編排", context_prompt)
        self.assertIn("主要任務模式執行代理", context_prompt)
        self.assertIn("招募流程、員工角色或人員分配", context_prompt)
        ...
        self.assertIn("## 任務模式編排", str(context_messages[0].get("content", "")))
        self.assertIn("Tool Strategy", str(context_messages[-1].get("content", "")))
        self.assertIn("包含簡短的驗證狀態", prompt)
    ...
    async def test_plan_subagent_uses_unified_prompt_without_task_mode_orchestration(self) -> None:
        ...
        self.assertEqual(task.metadata["runtime_prompt_profile"], "unified")
        self.assertIn("## 原生工作契約", prompt)
        self.assertIn("規劃時，產出決策完整的步驟", prompt)
    ...
    async def test_company_review_work_item_uses_company_contract_and_verify_profile(self) -> None:
        ...
        self.assertEqual(task.metadata["runtime_prompt_profile"], "unified")
        self.assertIn("## 原生工作契約", prompt)
        self.assertNotIn("## 公司工作項目契約", prompt)
        self.assertIn("## 公司工作項目契約", context_prompt)
        self.assertIn("Work item turn type: `review`", context_prompt)
        self.assertIn("## 公司工作項目輪次：審查", context_prompt)
