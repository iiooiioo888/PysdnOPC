...
class ExternalAgentMonitoringTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_codex_adapter_mirrors_user_auth_and_config_with_copy_fallback(self) -> None:
        ...
        try:
            ...
            self.assertEqual((agent_home / "auth.json").read_text(encoding="utf-8"), '{"OPENAI_API_KEY":"sk-test"}')
            self.assertEqual((agent_home / "config.toml").read_text(encoding="utf-8"), 'model_provider = "codex"\n')
        finally:
            _cleanup_test_dir(tmpdir)
    ...
    def test_opencode_adapter_mirrors_user_config_with_copy_fallback(self) -> None:
        ...
        try:
            ...
            self.assertEqual((agent_home / "opencode.json").read_text(encoding="utf-8"), '{"model":"test"}')
        finally:
            _cleanup_test_dir(tmpdir)
    ...
    def test_install_collab_surface_is_idempotent(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if os.name == "nt":
                ...
                self.assertIn("opc.cli_collab", cmd_shim.read_text(encoding="utf-8"))
            else:
            ...
            self.assertIn("opc-collab", skill.read_text(encoding="utf-8"))
